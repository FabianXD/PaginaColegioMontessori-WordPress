...
class Alg_Download_Plugins_Core {
	...
	function download_theme_all() {
		if ( isset( $_GET['alg_download_theme_all'] ) && is_user_logged_in() && current_user_can( 'switch_themes' ) ) {
			...
			if ( $this->create_zip( $args, $files ) ) {
				...
				add_action( 'admin_notices', array( $this, 'unknown_error_message' ) );
				return false;
			}
		}
	}
	 ...
	 * @since   1.4.0
	 */
	function unknown_error_message() {
	 ...
	 * @version 1.4.0
	 * @since   1.3.0
	 * @todo    [dev] maybe add option to manually select first/main `$zip_library`
	 * @todo    [dev] re-check returns
	 */
